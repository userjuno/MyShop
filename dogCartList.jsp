...
<%
	String co = (String)session.getAttribute("id");
	String cartUrl="";
if(co==null){
	%><script>
	alert("로그인 후 이용해 주세요");
...
<li style="padding: 15%;"><a style="color: white;width:80px" href="freeboardList.fbo">질문게시판</a></li>
    ...
    <div class="contents" style="padding-top: 70px;">
     <form  method="post" action="dogCartPay.dog" enctype="multipart/form-data">
   <% 
			String b = (String)session.getAttribute("id");
			System.out.println("세션 아이디" + b);
			String c = (String)session.getAttribute("pwd");
			bean = mgr.getMember(b, c);
			%>
		
        <div class="buybox">
       	 <input type="hidden" name="gg" value="제발 되라고">
            <div class="order_info">
                ...
                <div class="row">
                    ...
                    <div class="space_right">
                        <input class="order_name" type="text" title="이름" placeholder="이름" name="orderer_name" value=<%=bean.getName()%> >
                    </div>
                    
                    <div class="tel">
                        <input class="order_call" type="tel" title="연락처" placeholder="연락처" name="orderer_call" value=<%=bean.getPhone() %>>
                    </div>
                ...
                <br>
                    <h3>배송지 정보</h3>
                    <input name="zipcode" style="margin: 2.5% 0;" class="M_t zipcode"  type="text" id="sample6_postcode_2" placeholder="우편번호" value=<%=bean.getZipcode() %>>
                    <input style="margin: 2.5% 0;" class="M_b find_zip"  type="button" onclick="sample6_execDaumPostcode_2()" value="우편번호 찾기"><br>
                    <input name="address" style="margin: 2.5% 0;" class="M_t"  type="text" id="sample6_address_2" placeholder="주소" value=<%=bean.getAddress() %>><br>
                    <input name="addr_more" style="margin: 2.5% 0;" class="M_t"  type="text" id="sample6_detailAddress_2" placeholder="상세주소" value=<%=bean.getAddr_more() %>>
                    <input style="margin: 2.5% 0;" class="M_t"  type="text" id="sample6_extraAddress_2" placeholder="참고항목">
                    ...
                    <h3>배송 메모</h3>
                    <div class="space_top">
                        <input class="order_memo M_t" type="text" title="메모" placeholder="배송메모" name="memo" value="">
                    </div>
                    <div class="buy">
                    <h3>결제 수단</h3>
                    <input type="radio">무통장입금
                    <div class="space_right">
                        <input class="M_t" type="text" title="이름" placeholder="입금자명(미입력시 주문자명)" name="orderer_name" value=<%=bean.getName()%> >
                    </div>
                </div>
   ...
   <h2>장바구니 목록</h2>
      ...
      <table>
        ...
        <tr>
        	<td>
        		<input type="checkbox" id="remove" name="remove" value="${cart.c_name}"/>
        	</td>
             <td>
             ...
             <td>
             <input type="hidden" name="dog_img"  value="${cart.image }">
             <img src = "images/${cart.image }" name="cartImage" id ="cartImage"/>
            </td>
             <td>
             ${cart.c_name }
            </td>
             <td>
             <input type="hidden" name="item_price" value= ${cart.c_price }>
             ${cart.c_price }
            </td>
             <td>
             <a href="dogCartQtyUp.dog?kind=${cart.c_name }">
             <img src="images/up.jpg" id = "upImage" border=0/>
             </a><br>
             <input type="hidden" name="pay_count" value= ${cart.c_count }>
             ${cart.c_count }<br>
             <a href="dogCartQtyDown.dog?kind=${cart.c_name }">
             <img src="images/down.jpg" id = "downImage" border=0/>
             </a>
            </td>
            <td>
            <input type="hidden" name="pay_count" value= ${cart.c_count }>
            ${cart.c_price*cart.c_count}
            </td>
         </tr>
         <input class="mc" type="hidden" value="${cart.c_price*cart.c_count}">
    	<script>
         		var b = Number(b);
         		b+=${cart.c_price*cart.c_count};
      ...
      <tr>
         <td colspan="7" style="text-align:center;margin-left:15%;">
         	<input type="submit" value="삭제" formaction="dogCartRemove.dog"/>
         	<input id="chbtn" type="submit" value="구매	"/>
         	      <nav id="commandList">
