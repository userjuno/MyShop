...
<%@page import="mgrvo.PageInfo"%>
<%@ page import="java.util.*"%>
<%@page import="mgrvo.BoardBean"%>
...
<body onLoad="regFrm.id.focus();">
    ...
    <div class="contents">
       <div class="b_pan">
       		<div class="s_pan">
       			<ul style="padding:13px;font-size:0.8em">
       				<li><a class="my_tag" href="boardList.mo">1:1 문의 관리</a></li>
       				<li><a class="my_tag" href="boardInfoList.mo">회원 리스트</a></li>
       				<li><a class="my_tag" href="dogOrderInfo.dog">회원 구매 리스트</a></li>
		...
		<table style="margin-top:8%;margin-left:400px">
			<%
...
%>
	...
	%>
			<tr>
				...
				<td>
					...
					&nbsp; <%} %> ▶ <%}else{ %> ▶ <%} %> <a
					href="userboardDetail.ubo?board_num=<%=articleList.get(i).getBOARD_NUM()%>&page=<%=nowPage%>">
	...
	<div style="position:relative;margin-left:80%;">
	<a style="background-color:rgb(233,233,233);width:100px;height:20px;padding:13px" href="boardWriteForm.bo">게시판글쓰기</a>
	</div>

	<section id="pageList" style="margin-left:45%">
		<%if(nowPage<=1){ %>
		[이전]&nbsp;
		<%}else{ %>
		<a href="userboardList.ubo?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
		<%} %>
		...
		[<%=a %>]
		<%}else{ %>
		<a href="userboardList.ubo?page=<%=a %>">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="userboardList.ubo?page=<%=nowPage+1 %>">[다음]</a>
		<%} %>
	</section>
	<%
    }
