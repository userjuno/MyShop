<%@page import="fvo.PageInfo"%>
<%@page import="fvo.BoardBean"%>
...
<body onLoad="regFrm.id.focus();msg()">
      ...
      <div id="modal"></div>
        <div class="modal-con modal1">
          ...
          <div class="con">
            ...
            <button class="M_login" onclick="loginCheck()">로그인</button>
          	</form>
        ...
        <div class="modal-con modal3">
            ...
            <div class="con Mo_con">
                <form name="regFrm" method="post" action="memberProc.jsp" >
                    ...
                    <label class="l_name">이름<span>*<span></label>
...
<li style="padding: 15%;"><a style="color: white;width:80px" href="freeboardList.fbo">질문게시판</a></li>
	...
	<!-- 게시판 수정 -->
	<h1 style="margin-left:15%">공지사항</h1>
	<section id="articleForm">
	
		<section id="basicInfoArea"  style="line-height:40px ;">
			제 목 :
			<%=article.getBOARD_SUBJECT()%>
			
		</section>
		<p style="border:1px solid rgb(223,223,223);margin:0; padding:0;" >
			첨부파일 :
			<%if(!(article.getBOARD_FILE()==null)){ %>
				<a href="javascript:down('<%=article.getBOARD_FILE()%>')"><%=article.getBOARD_FILE()%></a>
			<%} %>
		</p>
		<section id="articleContentArea" style="background-color:rgb(233,233,233);padding:3%; padding-bottom:0;margin-top:0;text-align:left">
	...
	<section id="commandList">
		<a class="my_power"	href="freeboardReplyForm.fbo?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage%>">
			[답변] </a> 
			<a class="ur_power"	href="freeboardUpdateForm.fbo?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage%>">
			[수정] </a>
			 <a class="ur_power" href="freeboardDeleteForm.fbo?board_num=<%=article.getBOARD_NUM() %>&page=<%=nowPage%>">
			[삭제] </a> <a href="freeboardList.fbo?page=<%=nowPage%>">[목록]</a>&nbsp;&nbsp;
	</section>
	</div>
</body>
        <h1 style="margin-top: 5%; margin-left: 47%; margin-bottom: 5%;"></h1>
     <div id="comment">
    ...
    %>
     	   <section id="listForm" style="height:300px">
		
			글 목록<a href="freeboardWriteForm.fbo">게시판글쓰기</a>
...
%>
	...
	%>
			<tr>
				...
				<td>
					<%if(articleList.get(i).getBOARD_RE_LEV()!=0){ %> <%for(int ab=0;ab<=articleList.get(i).getBOARD_RE_LEV()*2;ab++){ %>
					&nbsp; <%} %> ▶ <%}else{ %> ▶ <%} %>
	...
	<section id="pageList">
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="freeboardList.fbo?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
		<%} %>

		<%for(int abc=startPage;abc<=endPage;abc++){
				if(abc==nowPage){%>
		[<%=abc %>]
		<%}else{ %>
		<a href="freeboardList.fbo?page=<%=abc %>">[<%=abc %>]
		</a>&nbsp;
		<%} %>
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="freeboardList.fbo?page=<%=nowPage+1 %>">[다음]</a>
		<%} %>
	</section>	
	<%
    }
...
%>
			...
			<tr bgcolor="#F5F5F5">
			<form action="commboardWritePro.cbo">
				<input type="hidden" name="BOARD_PNUM" value=<%=article.getBOARD_NUM() %>>
				<input type="hidden" name="board_BOARD_NAME" value="<%=a %>">
				
				<!-- 아이디-->
				<td width="150">
					<div>
						<%=a %>
						<input type="password" name="BOARD_PASS" placeholder="비밀번호 입력" >
					</div>
				</td>
				<!-- 본문 작성-->
				<td width="550">
					<div>
						<textarea name="BOARD_SUBJECT" rows="4" cols="70" ></textarea>
					</div>
				</td>
				<!-- 댓글 등록 버튼 -->
				<td width="100">
					<div id="btn" style="text-align:center;">
						<p><input type="submit" value="등록" ></a></p>	
					</div>
    ...
    $(".ur_power").css('display','none');
    $(".my_power").css('display','none');
    <%
    String writer = article.getBOARD_NAME();
