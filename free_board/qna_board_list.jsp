...
%>
	...
	%>
			<tr>
				...
				<td>
					...
					&nbsp; <%} %> ▶ <%}else{ %> ▶ <%} %> <a
					href="freeboardDetail.fbo?board_num=<%=articleList.get(i).getBOARD_NUM()%>&page=<%=nowPage%>">
	...
	<div style="position:relative;margin-left:70%;">
		<a style="background-color:rgb(233,233,233);width:100px;height:20px;padding:13px" href="freeboardWriteForm.fbo">게시판글쓰기</a>
	</div>
	...
	<section id="pageList">
		...
		[<%=a %>]
		<%}else{ %>
		<a href="freeboardList.fbo?page=<%=a %>">[<%=a %>]
		</a>&nbsp;
		<%} %>
		<%} %>
